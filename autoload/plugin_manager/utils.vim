...
" Execute command with async job (if available) or fallback to sync
function! plugin_manager#utils#execute_async(cmd, options)
  " Check if jobs are available
  if exists('*plugin_manager#jobs#has_async') && plugin_manager#jobs#has_async()
    " Execute asynchronously
    return plugin_manager#jobs#start(a:cmd, a:options)
  else
    " Fallback to synchronous execution
    let l:title = get(a:options, 'description', 'Executing command')
    let l:output = plugin_manager#utils#execute_with_sidebar(l:title, a:cmd)
    
    " If there's a completion callback, call it with the results
    if has_key(a:options, 'on_complete') && type(a:options.on_complete) == v:t_func
      call a:options.on_complete(v:shell_error, split(l:output, "\n"))
    endif
